  ...
  const fetchLogs = async () => {
    try {
      ...
      if (error.response && error.response.status === 403) {
         showAlert('Access Denied', 'Only Super Admins can view activity logs.', 'security');
      } else {
    ...
    if (window.confirm('Are you sure you want to remove this team member?')) {
      try {
        ...
        if (error.response && (error.response.status === 403 || error.response.status === 401)) {
            showAlert('Permission Denied', 'You do not have the authority to delete users.', 'security');
        } else {
            showAlert('Error', error.response?.data?.message || 'Failed to delete user.', 'error');
        }
      }
    }
  };
  ...
  const handleModalSubmit = async (formData) => {
      try {
        if (currentUser) {
          ...
          showAlert('Success', 'User profile updated.', 'success');
        } else {
          // Create Mode
          await api.post('/users', formData);
          showAlert('Success', 'New team member added.', 'success');
        }
        setShowModal(false);
        fetchUsers();
      } catch (error) {
        if (error.response && (error.response.status === 403 || error.response.status === 401)) {
            showAlert('Permission Denied', 'You do not have the authority to manage users.', 'security');
        } else {
            showAlert('Error', error.response?.data?.message || 'Operation failed.', 'error');
        }
      }
  };

  // --- UPDATED: Smart "Active" Status Logic ---
  const getUserStatus = (user) => {
    // REMOVED "|| user.lastLogin" to be stricter. 
    // Now, only the Heartbeat (lastActive) determines online status.
    if (!user.lastActive) return 'Inactive';

    const tenMinutes = 10 * 60 * 1000; 
    const isActiveNow = (new Date() - new Date(user.lastActive)) < tenMinutes;
    
    return isActiveNow ? 'Active' : 'Inactive';
};

  // --- UPDATED: Safe Filter Logic (Crash Fix) ---
  const filteredUsers = users.filter(user => {
    const term = searchQuery.toLowerCase();
    const name = user.name || '';   // Fallback to empty string
    const email = user.email || ''; // Fallback to empty string
    const role = user.role || '';   // Fallback to empty string
  ...
  return (
    <div className="users-page-container">
          ...
          <table className="users-table">
            ...
            <tbody>
              {filteredUsers.map(user => {
                 const status = getUserStatus(user); // Pass full user object now
                 return (
                    <tr key={user._id}>
                    <td>
                    ...
                    <td>
                        <span className={`role-badge ${user.role}`}>
                            ...
                            {(user.role || 'Unknown').replace('-', ' ').toUpperCase()}
                        </span>
                    </td>
      ...
      <style>{`
        ...
        .header-btn { display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; height: 48px; padding: 0 1.5rem; background: rgba(255, 255, 255, 0.05); color: white; border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 50px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; font-size: 0.9rem; white-space: nowrap; }
        .header-btn:hover, .header-btn.primary-btn:hover { background: #ccff00; color: black; border-color: #ccff00; box-shadow: 0 0 15px rgba(204, 255, 0, 0.4); transform: translateY(-2px); }
        ...
        /* --- NEW MODAL STYLES (Fix for Overlap) --- */
        .modal-overlay { 
        ...
        .activity-modal-content { 
            width: 100%; max-width: 700px; max-height: 70vh; 
            background-color: #0d1629; border: 1px solid rgba(255,255,255,0.15); 
        ...
        .user-modal-content {
            ...
            overflow-y: auto; background-color: #0d1629; 
            border: 1px solid rgba(255,255,255,0.15); border-radius: 16px; 
            padding: 1.5rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.8);
        }
        ...
        @media (max-width: 768px) {
            ...
            .users-table td:nth-child(2), .users-table td:nth-child(4) { display: inline-block; width: auto; padding-right: 1rem; }
            
            /* Modal Fix for Mobile */
            .activity-modal-content, .user-modal-content {
                margin-top: 4.5rem;
                max-height: 80vh;
            }
        }
    ...
    return (
        <div className="modal-overlay">
            <div className="activity-modal-content">
                ...
                {/* Added flex: 1 to ensure body expands to fill available space in column layout */}
                <div style={{ overflowY: 'auto', padding: '1rem', flex: 1 }}>
                    {logs.length === 0 ? (
                        ...
                        logs.map((log) => (
                            <div key={log._id} style={{ display: 'flex', gap: '0.75rem', padding: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                <div style={{ paddingTop: '0.2rem' }}>
                                    {log.action.includes('DELETE') ? <Trash2 size={16} color="var(--danger)" /> : log.action.includes('CREATE') ? <Plus size={16} color="var(--success)" /> : log.action.includes('LOGIN') ? <Clock size={16} color="var(--primary)" /> : <FileText size={16} color="var(--text-muted)" />}
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.25rem' }}>
                                        <span style={{ fontWeight: 600, color: 'white', fontSize: '0.9rem' }}>{log.user ? log.user.name : 'Unknown User'}</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{new Date(log.timestamp).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
    ...
    <div className="modal-overlay">
      <div className="user-modal-content">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1rem' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', fontFamily: 'var(--font-neon)', margin: 0, color: 'white' }}>{user ? 'Edit Member' : 'Add Member'}</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', color: '#cbd5e1', fontSize: '0.85rem', fontWeight: '500' }}>Password {user && <span style={{ fontSize: '0.75rem', opacity: 0.7 }}>(Leave blank to keep current)</span>}</label>
